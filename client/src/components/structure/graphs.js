...
export function PieChartLoad({nitrogen, phosphorus, ammonium}){
  ...
  const phosphorusLoad = phosphorus.map(region => region.Total_Phosphorus_Load);
...
export function PieChartConcentration({nitrogen, phosphorus, ammonium}){
  const nitrogenConcentration = nitrogen.map(region => region.Total_Nitrogen_Concentration);
  const phosphorusConcentration = phosphorus.map(region => region.Total_Phosphorus_Concentration);
  const ammoniumPhosphorus = ammonium.map(region => region.Ammonia_Nitrogen_Concentration);
    ...
    series: [
      {
        ...
        radius: '50%',
        data: [
          { value: nitrogenConcentration, name: 'Total Nitrogen Concentration' },
          { value: phosphorusConcentration, name: 'Total Phosphorus Concentration' },
          { value: ammoniumPhosphorus, name: 'Total Ammonia Nitrogen Concentration' }
        ],
        emphasis: {
          itemStyle: {
            shadowBlur: 10,
...
export function ScatterChart({data1, type}){
  const schema = [
    ...
    { name: `Total_${type}_Concentration`, index: 7, text: `Total ${type} Concentration` }
  ];
    ...
    tooltip: {
      ...
      formatter: function (param) {
        ...
        return '<div style="border-bottom: 1px solid rgba(255,255,255,.3); font-size: 18px;padding-bottom: 7px;margin-bottom: 7px">'
                  + 'Region ' + value[0] + '<br>'
                  + schema[2].text + ': ' + value[2] + ' cms<br>'
                  + schema[7].text + ': ' + value[7] + ' mg/L<br>'
                  + schema[1].text + ': ' + value[1] + ' km²<br>'
                  + schema[3].text + ': ' + value[3] + ' kg<br>'
                  + schema[4].text + ': ' + value[4] + ' m³<br>'
...
export function GaugeChart({data, type}){
  ...
  const _type = type != 'Ammonium Nitrogen'? 'Total_'+type : 'Ammonia_Nitrogen';
  
  const totalNitrogenConcentration = data[currentRegion][`${_type}_Concentration`];
